...
sinput string         Info_Support         = "cindo.pages.dev";   //--- Dashboard Layout
...
input int             Panel_Width          = 600;      // Total Dashboard Width//--- Trading Engine Settings
...
input int             InpMagic             = 888999;        // Algorithm Serial ID//--- Theme & Color Settings
...
input color           Live_Color           = C'255,225,100';    // Real-time Accent Color//--- Chart Visualization
...
CTrade   trade;// Sequential State Machine
double   g_lastLot      = 0;  // Volume dari posisi terakhir yang ditutup
int      g_lastDeal      = -1; // Ticket deal terakhir yang telah diproses
bool     g_isFirstTrade = true; // Flag untuk perdagangan pertama#define PREFIX          "GRAFIKTABRANIJ"
...
#define COLOR_SILVER    C'180,180,180' // Muted Silver#define ROW_H           30
#define FONT_MAIN       "Segoe UI"
#define FONT_SIZE       10
#define COLOR_COUNTDOWN C'255,200,50'   // Amber Gold (Countdown)
#define VIS_PREFIX      "GT_VIS_"// Global State for UI Tabs
enum ENUM_TABS {
    TAB_DASHBOARD,
    TAB_ABOUT,
    TAB_TRADING,
...
ENUM_TABS currTab = TAB_DASHBOARD;// Global Color Theme Variables
color gClrBg, gClrHdr, gClrStripe, gClrLabel, gClrValue, gClrAccent, gClrSuccess, gClrDanger;// Runtime Modifiable Settings (Mirrored from Inputs)
ENUM_THEME   extTheme;
bool         extShowGTChart;// Wall-clock synchronization
long         serverLocalOffset = 0;void ApplyTheme()
{
   switch(extTheme)
   {
...
{
   myPoint  = _Point;
   myDigits = _Digits;   // Load persistent settings from Terminal Global Variables if they exist
   ...
   if(GlobalVariableCheck(themeGV))
      ...
      extTheme = InpTheme;   string showChartGV = PREFIX + "_" + _Symbol + "_" + IntegerToString(InpMagic) + "_SHOWCHART";
   if(GlobalVariableCheck(showChartGV))
      extShowGTChart = (bool)GlobalVariableGet(showChartGV);
   else
      extShowGTChart = InpShowGTChart;   // Calculate offset between Server Time and Local Machine Time
   serverLocalOffset = (long)TimeCurrent() - (long)TimeLocal();   ApplyTheme();
   DeleteAllObjects();   if(!CreateDashboard())
   {
...
{ 
   EventKillTimer(); 
   DeleteAllObjects(); 
   DeleteVisualization();    // Clean up Global Variables only if the EA is explicitly removed or chart is closed
...
{ 
   UpdateGUILabels();    // Sequential State Machine - check every tick for empty position
   ExecuteTradingLogic();   if(extShowGTChart) DrawGTLevels();
}
void OnTimer()                   { UpdateCountdown(); }void OnChartEvent(const int id, const long& lparam, const double& dparam, const string& sparam)
{
   if(id == CHARTEVENT_OBJECT_CLICK)
   {
      ...
      else if(sparam == PREFIX + "TAB_VS") { currTab = TAB_VISUAL;    ResetDashboard(); }  // Theme Switching Logic (from Colors Tab)
...
}void DrawGTLevels()
{
   ...
   double low   = iLow(_Symbol, PERIOD_CURRENT, 0);   if(open == 0) return;   double bH = MathMax(open, close), bL = MathMin(open, close);   DrawHLine(VIS_PREFIX + "Tinggi", high,  gClrAccent, STYLE_DOT);
...
}void CreateQuadBarRow(string prefix, int y, string label)
{
   ...
   int colW      = (Panel_Width - dataStart) / 4;   // Label column
   CreateLabel(prefix + "_lbl", X_Offset + 20, textY, label, gClrLabel);   // 4 Value Columns (Ultra-Compact)
...
}void ResetDashboard() { DeleteAllObjects(); CreateDashboard(); }bool CreateDashboard()
{
   int y = Y_Offset;
   int totalH = (currTab == TAB_DASHBOARD) ? 525 : 410;   CreateRect(PREFIX + "Main", X_Offset, y, Panel_Width, totalH, gClrBg, clrDimGray);
   CreateCornerBrackets(X_Offset - 2, y - 2, Panel_Width + 4, totalH + 4, 15, gClrAccent);   // Tab Navigation Bar
   int tabW = (Panel_Width - 20) / 5;
   int tabX = X_Offset + 10;
   int tabY = y + 10;   CreateTabButton(PREFIX + "TAB_DB", tabX + (tabW/2),       tabY, tabW, 25, "DASHBOARD", currTab == TAB_DASHBOARD);
   ...
   CreateTabButton(PREFIX + "TAB_VS", tabX + (tabW*4 + tabW/2), tabY, tabW, 25, "VISUAL",    currTab == TAB_VISUAL);   y += 45;   if(currTab == TAB_DASHBOARD) CreateDashboardTab(y);
   ...
   else if(currTab == TAB_VISUAL) CreateVisualTab(y);   ChartRedraw();
   return true;
}void CreateDashboardTab(int y)
{
   ...
   y += 40;   CreateQuadBarRow(PREFIX + "R_OH",    y, "Tinggi");    y += ROW_H;
   ...
   CreateQuadBarRow(PREFIX + "R_Range", y, "Jangkauan"); y += ROW_H + 5;   UpdateInfoSectionOnDashboard(y);
}void UpdateInfoSectionOnDashboard(int y)
{
   CreateRect(PREFIX + "InfoBg", X_Offset + 4, y, Panel_Width - 8, 135, gClrStripe);
   int infoY = y + 10;   // Row 1: Balance & Equity
   ...
   CreateLabel(PREFIX + "Acc_EqVal", X_Offset + Panel_Width - 20, infoY, "0.00", gClrValue, 8, FONT_MAIN, ANCHOR_RIGHT_UPPER);   // Row 2: Spread & Symbol P/L
   infoY += 22;
   ...
   CreateLabel(PREFIX + "Sym_PLVal", X_Offset + Panel_Width - 20, infoY, "0.00", gClrAccent, 8, FONT_MAIN, ANCHOR_RIGHT_UPPER);   // Row 3: Buy Exposure & Margin Level
   infoY += 22;
   ...
   CreateLabel(PREFIX + "Acc_MLVal", X_Offset + Panel_Width - 20, infoY, "0.00%", gClrValue, 8, FONT_MAIN, ANCHOR_RIGHT_UPPER);   // Row 4: Sell Exposure & Eq to SO
   infoY += 22;
   ...
   CreateLabel(PREFIX + "Acc_SOEqVal", X_Offset + Panel_Width - 20, infoY, "0.00", gClrValue, 8, FONT_MAIN, ANCHOR_RIGHT_UPPER);   // Row 5: SO Price & Pts to SO
   infoY += 22;
...
}void CreateAboutTab(int y)
{
   ...
   CreateRect(PREFIX + "AboutBg", X_Offset + 4, y, Panel_Width - 8, 340, gClrStripe);   int lineY = y + 20;
   ...
   lineY += 20;
   CreateLabel(PREFIX + "Ab_Desc2", contentX, lineY, "Ketakutan akan masa depan yang buruk dan kenangan akan masa lalu yang buruk", clrWhite, 9);   lineY += 40;
   ...
   CreateLabel(PREFIX + "Ab_MethodVal", contentX + 120, lineY, "Grafik Tabranij (GT) Matematika Pasar", gClrValue, 8);   lineY += 50;
...
}void CreateTradingTab(int y)
{
   ...
   int lineY = y + 20;
   int contentX = X_Offset + 20;   CreateLabel(PREFIX + "Tr_Title", contentX, lineY, "RINGKASAN PENGATURAN ALGORITMA", gClrAccent, 10);
   lineY += 40;   CreateLabel(PREFIX + "Tr_StratL", contentX, lineY, "Durasi GT:", gClrLabel, 9);
   ...
   CreateLabel(PREFIX + "Tr_TPV", contentX + 150, lineY, IntegerToString(InpTP) + " / " + IntegerToString(InpSL) + " pts", gClrValue, 9);   lineY += 50;
...
}void CreateColorsTab(int y)
{
   ...
   int lineY = y + 20;
   int centerX = X_Offset + Panel_Width/2;   CreateLabel(PREFIX + "Cl_Title", centerX, lineY, "PILIH TAMPILAN TEMA", gClrAccent, 11, FONT_MAIN, ANCHOR_CENTER);
   lineY += 50;   int btnW = 180, btnH = 35;
   ...
   lineY += 50;
   CreateButton(PREFIX + "THM_MATRIX", centerX, lineY, btnW, btnH, "RETRO MATRIX", clrWhite, C'10,50,10');   lineY += 60;
...
}void CreateVisualTab(int y)
{
   ...
   int lineY = y + 20;
   int centerX = X_Offset + Panel_Width/2;   CreateLabel(PREFIX + "Vs_Title", centerX, lineY, "PENGATURAN TAMPILAN GT", gClrAccent, 11, FONT_MAIN, ANCHOR_CENTER);
   lineY += 60;   string toggleText = extShowGTChart ? "MENONAKTIFKAN LEVEL GT" : "MENGAKTIFKAN LEVEL GT";
   color toggleBg = extShowGTChart ? gClrDanger : gClrSuccess;   CreateButton(PREFIX + "TOG_CHART", centerX, lineY, 220, 40, toggleText, clrWhite, toggleBg);   lineY += 80;
...
void UpdateGUILabels()
{
   ...
   UpdateBarData(0, "_v0", gClrAccent);   UpdateInfoSection();
   ChartRedraw();
}void UpdateInfoSection()
{
   double balance = AccountInfoDouble(ACCOUNT_BALANCE);
   double equity  = AccountInfoDouble(ACCOUNT_EQUITY);
   int    spread  = (int)SymbolInfoInteger(_Symbol, SYMBOL_SPREAD);   double symbolPL = 0;
   double buyLots = 0;
   double sellLots = 0;
   double buyPriceSum = 0;
   double sellPriceSum = 0;   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ...
      if(PositionSelectByTicket(ticket))
      {
         if(PositionGetString(POSITION_SYMBOL) == _Symbol)
         {
            ...
            ENUM_POSITION_TYPE posType = (ENUM_POSITION_TYPE)PositionGetInteger(POSITION_TYPE);        if(posType == POSITION_TYPE_BUY)
        {
           buyLots += lots;
   ...
   double avgSellPrice = (sellLots > 0) ? sellPriceSum / sellLots : 0;   SetVal(PREFIX + "Acc_BalVal", DoubleToString(balance, 2), gClrValue);
   SetVal(PREFIX + "Acc_EqVal", DoubleToString(equity, 2), gClrValue);
   SetVal(PREFIX + "Sym_SpreadVal", IntegerToString(spread), gClrValue);   // Calculate point value total early
   ...
   double pointValueTotal = MathAbs(netLots) * pointValuePerLot;   double ptsFloating = 0;
   if(pointValueTotal > 0)
      ptsFloating = symbolPL / pointValueTotal;   // Format Symbol P/L dynamically with signs, unicode indicators, and neutral state color
   string plStr = "";
   color plColor = gClrValue;   if(symbolPL > 0.005)
   {
      plColor = gClrSuccess;
   ...
   SetVal(PREFIX + "Sym_PLVal", plStr, plColor);   // Risk Analysis Calculations
   double margin = AccountInfoDouble(ACCOUNT_MARGIN);
   double marginLevel = AccountInfoDouble(ACCOUNT_MARGIN_LEVEL);
   double stopOutLevel = AccountInfoDouble(ACCOUNT_MARGIN_SO_SO);
   long stopOutMode = AccountInfoInteger(ACCOUNT_MARGIN_SO_MODE);   string buyExpStr = "0.00 Lots";
   string sellExpStr = "0.00 Lots";
   string ptsToSOStr = "-";
   string eqToSOStr = "-";
   string marginLevelStr = "0.00%";
   string stopOutPriceStr = "-";
   color riskColor = gClrValue;   int dispDigits = (myDigits > 4) ? 4 : myDigits;
   string buyPriceStr = DoubleToString(avgBuyPrice, dispDigits);
   string sellPriceStr = DoubleToString(avgSellPrice, dispDigits);   if(buyLots > 0)
      buyExpStr = StringFormat("%.2f @ %s ▲", buyLots, buyPriceStr);
   if(sellLots > 0)
      sellExpStr = StringFormat("%.2f @ %s ▼", sellLots, sellPriceStr);   if(buyLots > 0 || sellLots > 0)
   {
      if(margin > 0)
      {
         marginLevelStr = StringFormat("%.1f%% (SO: %.1f%%)", marginLevel, stopOutLevel);     if(marginLevel < 150.0)
...
}void UpdateBarData(int shift, string suffix, color baseClr)
{
   ...
   double low   = iLow(_Symbol, PERIOD_CURRENT, shift);   if(open == 0) return;   int oc = (int)((close - open) / myPoint);
   int lh = (int)((high - low) / myPoint);
   double bH = MathMax(open, close), bL = MathMin(open, close);
   int ch = (int)((high - bH) / myPoint), cl = (int)((bL - low) / myPoint);   SetVal(PREFIX + "R_OH" + suffix, DoubleToString(high, myDigits), baseClr);
...
void UpdateCountdown()
{
   ...
   int periodSeconds = PeriodSeconds(tf);   // Gunakan waktu lokal + offset untuk mendapatkan "Server Time" yang presisi per detik
   datetime serverTimeLive = (datetime)((long)TimeLocal() + serverLocalOffset);   datetime barOpenTime = iTime(_Symbol, tf, 0);
   int elapsed   = (int)(serverTimeLive - barOpenTime);
   int remaining = periodSeconds - elapsed;   if(remaining < 0) remaining = 0;   int hours   = remaining / 3600;
   int minutes = (remaining % 3600) / 60;
   int seconds = remaining % 60;   string countdownText;
   if(hours > 0)
      countdownText = StringFormat("%02d:%02d:%02d", hours, minutes, seconds);
   else
      countdownText = StringFormat("%02d:%02d", minutes, seconds);   // Efek pulse: warna berkedip saat mendekati penutupan (< 60 detik)
   color cdColor;
   if(remaining <= 10)
   ...
   else if(remaining <= 60)
   {
      ...
      cdColor = COLOR_COUNTDOWN;       // Amber Gold normal   SetVal(PREFIX + "CountdownIcon", "p", cdColor);
...
ulong GetLastClosedDeal(double &outProfit, ENUM_POSITION_TYPE &outType)
{
   ...
   int total = HistoryDealsTotal();   for(int i = total - 1; i >= 0; i--)
   {
      ulong ticket = HistoryDealGetTicket(i);
      if(HistoryDealGetInteger(ticket, DEAL_MAGIC)  != (long)InpMagic) continue;
      if(HistoryDealGetString(ticket, DEAL_SYMBOL)  != _Symbol) continue;
      ENUM_DEAL_ENTRY entry = (ENUM_DEAL_ENTRY)HistoryDealGetInteger(ticket, DEAL_ENTRY);
      if(entry != DEAL_ENTRY_OUT && entry != DEAL_ENTRY_INOUT) continue; // only closing deals  outProfit = HistoryDealGetDouble(ticket, DEAL_PROFIT) + 
...
bool PlaceOrder(ENUM_POSITION_TYPE type, double lot, string comment)
{
   ...
   double price, sl, tp;
   bool result = false;   if(type == POSITION_TYPE_BUY)
   {
      price = ask;
   ...
   else
   {
      ...
      Print("GT EA: Order FAILED – ", trade.ResultRetcodeDescription());   return result;
}//--- Main Logic: Sequential State Machine
void ExecuteTradingLogic()
{
   // If we already have an open position, do nothing. Let broker SL/TP handle it.
   if(CountMyPositions() > 0) return;   // --------------- No open position: decide next action ---------------
   double lastProfit = 0;
   ENUM_POSITION_TYPE lastType = POSITION_TYPE_BUY;
   ulong lastDealTicket = GetLastClosedDeal(lastProfit, lastType);   double nextLot;
   ENUM_POSITION_TYPE nextType;   if(lastDealTicket == 0 || lastDealTicket == (ulong)g_lastDeal)
   {
      ...
      double lastClose = iClose(_Symbol, InpGTTimeframe, 1);  if(lastOpen == 0 || lastClose == 0) return;
  ...
  g_isFirstTrade = false;   }
   else
   {
      ...
      g_lastDeal = (int)lastDealTicket;  // Tentukan besarnya langkah Martingale berdasarkan dari rentetan kekalahan
  ...
  else
  {
   ...
   nextLot = NormalizeDouble(MathRound(nextLot / stepLot) * stepLot, 2);   string comment = StringFormat("GT|Step%d|Lot%.2f", lossStreak_or_new(), nextLot);
